...
def update_plot():
    ...
    with h5py.File(filename, 'r') as f:
        data = dict(x=f[group.value][key_property_x.value][:], y=f[group.value][key_property_y.value][:],)
...
def data_bin_change(attr, old, new):
    ...
    with h5py.File(filename, 'r') as f:
        ...
        x = np.nan_to_num(f[group.value][key_property_x.value][:] * 1, nan=0, posinf=0, neginf=0)

        y = np.nan_to_num(f[group.value][key_property_y.value][:] * 1, nan=0, posinf=0, neginf=0)
...
color_bar = ColorBar(color_mapper=cmap['transform'], location=(0, 0), ticker=LogTicker(), label_standoff=12,)
...
def update_stats(attr, old, new):
    with h5py.File(filename, 'r') as f:
        ...
        x_inf = len(np.argwhere(np.isinf(f[group.value][key_property_x.value][:])))
        y_inf = len(np.argwhere(np.isinf(f[group.value][key_property_y.value][:])))

        x_neginf = len(np.argwhere(np.isneginf(f[group.value][key_property_x.value][:])))
        y_neginf = len(np.argwhere(np.isneginf(f[group.value][key_property_y.value][:])))

        x_nan = len(np.argwhere(np.isnan(f[group.value][key_property_x.value][:])))
        y_nan = len(np.argwhere(np.isnan(f[group.value][key_property_y.value][:])))
